    ...
    while t < endTime - 1:
        ...
        for video in video_list:
            if video.start_time <= t:
                if video.end_time > maxi and video.end_time > t:
                    vid = video.video_clip
                    if vid not in aud_list:
                        maxi = video.end_time
        ...
        if check_overlap == 0:
            break
            video = video_list[count_videos]
            vid = video.video_clip
            maxi_start = video.start_time
            maxi = video.end_time
            video_save = video
            print("inside check overlap")
        aud_list.append(video_save.video_clip)
        index_start.append(maxi_start)
        index_list.append(t)
        t = maxi - 1
        count_videos = count_videos + 1

    file_vid = open(sys.argv[1] + "/mashup/audio_names.txt", "w")
    for ele in aud_list:
        file_vid.write(ele)
        file_vid.write("\n")
    """
