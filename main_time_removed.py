    ...
    if audio1.duration_seconds > 45:
        ...
        while t< audio1.duration_seconds:
            if t+40 < audio1.duration_seconds:
                ...
                clip.write_videofile(file_name + "_" + str(count) + ".mp4", codec="libx264")
...
for i in range(len(file_names)):
    ...
    while j < len(file_names):
        ...
        file_name1 = file_names[i].split('.')[0]
        file_name2 = file_names[j].split('.')[0]
        if file_name1[:-1] == file_name2[:-1]:
            file_name1 = file_name1[-1]
            file_name2 = file_name2[-1]
            if abs(int(file_name1)-int(file_name2)) == 1:
                is_match = 1
        if is_match > 0:
            print("Match")
...
def initialise_list(current_ele):
    for ele in dict_matches[current_ele]:
        ...
        if ele[1] > 0:
            ...
            # counting burnt pixels and using them as measure for video quality
            vidPath = folder_name + ele[0]
            cam = cv2.VideoCapture(vidPath)
            ret,frame = cam.read()
            if ret:
                dis = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
            rows,cols = dis.shape
            countDeadPix = float(np.count_nonzero(dis < 5) + np.count_nonzero(dis > 250))/(rows*cols)
            videoQuality = countDeadPix
            print (vidPath, countDeadPix)
            
            video_clip = Video(folder_name + ele[0], start_time,start_time + clip.duration, videoQuality)
            list_videos.append(video_clip)
